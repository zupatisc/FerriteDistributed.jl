...
function Ferrite.apply_zero!(K::PartitionedArrays.PSparseMatrix, f::PartitionedArrays.PVector, ch::ConstraintHandler)
    ...
    return map(local_values(K), local_values(f)) do K_local, f_local
        for cdof in pdofs
            K_local[cdof, :] .= 0.0
            K_local[:, cdof] .= 0.0
...
function Ferrite.apply!(K::PartitionedArrays.PSparseMatrix, f::PartitionedArrays.PVector, ch::ConstraintHandler)
    ...
    for part in remote_ghost_parts
    ...
    for (i, remote_ghost_dof) in enumerate(remote_ghosts_recv)
    ...
    return map(local_values(K)) do K_local
        for cdof in remote_ghost_ldofs[remote_ghosts_constrained_recv .== 1]
            K_local[:, cdof] .= 0.0
        end
    end
end

function Ferrite.apply!(u::PartitionedArrays.PVector, ch::ConstraintHandler)
    perm = _nod_to_oag_perm(ch.dh) # Mapping of DoFH indicies to PVector local indicies
    pdofs = perm[ch.prescribed_dofs]

    consistent!(u)
    #u_constrained = PartitionedArrays.pzeros(K.col_partition)
    return map(local_values(u)) do u_local
        u_local[pdofs] .= ch.inhomogeneities
        # v[ch.prescribed_dofs] .= apply_zero ? 0.0 : ch.inhomogeneities

        for (dof, dofcoef, h) in zip(pdofs, ch.dofcoefficients, ch.affine_inhomogeneities)
            dofcoef === nothing && continue
            @assert h !== nothing
            u_local[dof] = h
            for (d, s) in dofcoef
                u_local[dof] += s * u_local[perm[d]]
            end
        end
    end
    ...
    # for (dof, dofcoef, h) in zip(ch.prescribed_dofs, ch.dofcoefficients, ch.affine_inhomogeneities)   # Iterate through prescribeddofs, coefficients for the dofs and affine_inhomongeneities
    #     dofcoef === nothing && continue   # If the dof coefficient is nothing, skip
    #     @assert h !== nothing     # Inhomogeneities need to be something too
    #     v[dof] = apply_zero ? 0.0 : h     # set dof in v to affine_inhomogeneity
    #     for (d, s) in dofcoef     # Coefficients is a list of tubles?
    #         v[dof] += s * v[d]    # grab a value of a different dof from the vector and add it after multiplication with a coeficcient
    #     end
    # end
    # return v
    ####
