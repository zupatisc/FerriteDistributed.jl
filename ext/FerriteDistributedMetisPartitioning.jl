...
function FerriteDistributed.create_partitioning(grid::Grid{dim, C, T}, grid_topology::CoverTopology, n_partitions::Int, partition_alg::PartitioningAlgorithm.Metis) where {dim, C, T}
    ...
    @inbounds for i in 1:n_cells_global
        n_neighbors = 0
        for neighbor in getneighborhood(grid_topology, grid, CellIndex(i))
...
# TODO:(patrik) Very crude replication, merge this into the original function
function FerriteDistributed.create_partitioning(grid::Ferrite.AMR.NonConformingGrid{dim, C, T}, grid_topology::CoverTopology, n_partitions::Int, partition_alg::PartitioningAlgorithm.Metis) where {dim, C, T}
    n_cells_global = getncells(grid)
    @assert n_cells_global > 0
